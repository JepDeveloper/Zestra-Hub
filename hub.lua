...
if not game:IsLoaded() then
	local loadtext = "Fear Hub is waiting on the game to load"
...
Domain.Text.Text = "Fear Hub BETA 0.1"
...
Domain.Content.Text = "Hey fellow exploiter! We have detected an specific script for this game!"
...
Domain.Credits_4.Text = "Fear X  BETA V1"
...
Domain.About.Text = "Fear: Version 1,"
...
Domain.About_2.Text = "Brought to you buy Fear Technologies"
...
Domain.About_4.Text = "Hey fellow script/exploit showcaser! If you're showcasing this you'll get a role in the server!"
...
Domain.Credits_5.Text = "Fear hub BETA V1"
...
Domain.VersionText.Text = "You're running Fear hub version versoionnum"
...
Domain.text.Font = Enum.Font.GothamBold
Domain.text.Text = "Enjoying Fear hub? Join our Discord for updates and news on the latest releases"
Domain.text.TextColor3 = Color3.fromRGB(255, 255, 255)
Domain.text.TextScaled = true
Domain.text.TextSize = 14.000
Domain.text.TextWrapped = true
...
Domain.link.Text = "https://discord.gg/DPcdfgR6"
Domain.link.TextColor3 = Color3.fromRGB(255, 255, 255)
Domain.link.TextScaled = true
Domain.link.TextSize = 14.000
Domain.link.TextWrapped = true
...
Domain.SmallMessage.Text = "imagine not using fear hub"
...
ReleaseType = "Release"
Release = 1.39
ReleaseFeature = "Open source"
DiscordLink = "https://discord.gg/fcyrf88m2Tc"
DomainPass = 16104485
...
Banned = {17241083,26892987,20655358,10516000,660242324,225493776,529477452}
Developers = {304343782}
loldiscord = {"use fear x now - f34r","i spent lot of time on this - catware","showcase this","watch this space","bruh","use magnius - rexi","fear x winning","make it trending on twitter","oops","if this isnt in your auto exec, i will find you","preparing for lift off","dababy car","underrated","sirmeme make a video on fear hub","you know how i be, i just poopin","this table is long dude","discord.gg/shlex","boost the server damn it","pog champ","imagine trying to crack fear hub"}
...
BackgroundExploits = {
	...
	scp_roleplay = {
		Name = "SCP Roleplay gui!",
		Description = "Hey fellow exploiter! We got the gui!",
		PlaceIds = {5041144419},
		PremiumOnly = false,
		WaitDuration = 0,
...
UniversalExploits = {
	...
	dexex = {
		...
		Author = "Raspberry Pi",
		Colour = Color3.fromRGB(60, 60, 60),
		Code = "https://raw.githubusercontent.com/iCyphyDev/Dex-Explorer-V4/main/EEE.lua"
	},
	esp = {
		Name = "ESP",
		Author = "Synapse LLC Softworks",
		Colour = Color3.fromRGB(35, 35, 35),
		Code = "https://pastebin.com/raw/V7meMhzr"
	},
	axishub = {
		Name = "Axis Hub",
		Author = "Axis Technologies",
		Colour = Color3.fromRGB(60, 60, 60),
		Code = "https://raw.githubusercontent.com/iCyphyDev/Axis-hub/main/axishubui.lua"
	},
	owlhub = {
		Name = "Owl Hub",
		Author = "ZinityDrops",
		Colour = Color3.fromRGB(70 ,70 ,70),
		Code = "https://raw.githubusercontent.com/ZinityDrops/OwlHubLink/master/OwlHubBack.lua"
	},
	noclip = {
		Name = "PresidentIvan's Noclip",
		Author = "PresidantIvan",
		Colour = Color3.fromRGB(70 ,70 ,70),
		Code = "https://pastebin.com/raw/w0XEUW3y"
	},
}
...
function Load()
	Domain.LoadingFrame.Text.Text = "Fear Hub - 0.1"
		...
		for _, scrid in pairs(scr.PlaceIds) do
			if scrid == game.PlaceId then
				if scr.PremiumOnly == true then
					if game.Players.LocalPlayer:IsInGroup(7229632) then
						RePromptExploit(scr)
					end
				else
					RePromptExploit(scr)
...
function StartUnfunctionals()
	...
	Domain.VersionText.Text = "You're running Fear 0.1 "..ReleaseType.. " version "..tostring(Release)..[[    ]]
...
function StartServerhop()
	...
	elseif servers == "Protocol:found" then
		...
		Domain.ServerhopAnim.NoticeMessage.Text = "Fear hub found a server for you, please give us a moment"
...
function OpenAboutPlayer(Player)
	if Player then
		...
		if Player:IsInGroup(10220078) then
			Domain.DomainRole.Text = "Fear Premium"
			Domain.DomainRole.Visible = true
		end
		if table.find(Admins,Player.UserId) then
			Domain.DomainRole.Text = "Fear Staff"
			Domain.DomainRole.Visible = true
		end
		if table.find(Developers,Player.UserId) then
			Domain.DomainRole.Text = "Fear Developer"
			Domain.DomainRole.Visible = true
		end
		for _, customtitle in ipairs(customtitles) do
			for _, userid in ipairs(customtitle.userids) do
...
function BootDomain()
	if DOMAIN_ENABLED == true then
		...
		Notify("This is the beta version of fear hub! Keep yourself up-to-date by joining  the discord!","GothamBlack",Color3.fromRGB(0, 78, 115))
		wait(0.3)
		Notify("Running Fear Hub BETA","GothamSemibold",Color3.fromRGB(154, 18, 222))
		local Player = game.Players.LocalPlayer
		
		if table.find(Banned,Player.UserId) then
			Notify("Seems like you have been banned from Fear Hub, Booting Fear Hub V2 So you can't use it.","GothamBlack",Color3.fromRGB(170, 0, 0))
			LoadDomainV1()
	...
	StartUnfunctionals()
		...
		if game.Players.LocalPlayer:IsInGroup(10220078) then
			Notify("Welcome, "..Player.DisplayName.." to Fear Hub Premium 0.1!","GothamSemibold",Color3.fromRGB(46, 136, 111))
	...
	for _, Player in ipairs(game.Players:GetChildren()) do
		...
		Notify("Oof, fear is not enabled at this time, try again later!","GothamSemibold",Color3.fromRGB(184, 0, 0))
	end
end
...
Domain.Main.Position = UDim2.new(1.001,0,0.262,0)
Domain.Main.Shadow.ImageTransparency = 1
Domain.VersionText.Text = "You're running Fear Hub "..ReleaseType.. " version "..tostring(Release)..[[    ]]
Domain.VersionText2.Text = "- "..ReleaseFeature
Domain.ToggleButton.Rotation = 90
Domain.ToggleButton.ImageTransparency = 1
